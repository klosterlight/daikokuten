...
class AuctionBase extends React.Component {
	...
	componentDidMount() {
		...
		this.props.firebase.getAuctionBidUsers(this.props.match.params.id).onSnapshot((querySnapshot) => {
			let users = [];
			querySnapshot.forEach((doc) => {
				const user = doc.data();
				users.push(<li key={user.userId}>{user.displayName} - {user.email} - {moment.unix(user.bidAt.seconds).format("DD/MM/YYYY hh:mm:ss")}</li>);
			});
		...
		this.props.firebase.getAuctionBoughtUsers(this.props.match.params.id).onSnapshot((querySnapshot) => {
			let users = [];
			querySnapshot.forEach((doc) => {
				const user = doc.data();
				console.log(user);
				users.push(<li key={user.userId}>{user.displayName} - {user.email} - {moment.unix(user.boughtAt.seconds).format("DD/MM/YYYY hh:mm:ss")}</li>);
			});
					...
					<section className="inner-banner">
						<div className="container">
							...
							<ul className="thm-breadcrumb">
								...
								<li className="thm-breadcrumb__item">
									<a href="/admin/auctions" className="thm-breadcrumb__link">Subastas</a>
								</li>
								...
								<div className="col-lg-8">
									<div className="blog-details__content">
										...
										<div className="flexCol">
											<h3 className="blog-details__title mb-0">{this.state.auction.title}</h3>
										</div>
										...
										<div className="flexCol">
											<h3 className="blog-details__title mb-0">Usuarios que han subastado</h3>
											<ul>
												{this.state.users}
											</ul>
										</div>
										<div className="flexCol">
											<h3 className="blog-details__title mb-0">Usuarios que han comprado</h3>
											<ul>
												{this.state.usersBought}
											</ul>
										</div>
									</div>
								</div>
